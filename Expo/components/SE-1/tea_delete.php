...
$gro = "SELECT * FROM teacher";
$group = mysqli_query($con, $gro);
?>

<!DOCTYPE html>
<body>
<form method="POST" action="tea_delete_BN.php">
    ...
    <td style="background-color: #7c0000">
            <h5 align="center" style="font-weight: bold ; color:white">เลือกอาจารย์ที่ต้องการลบ</h5>
        </td>
    <td>
                        <select name="select_teacher1" style="width:60mm; margin-left:10%;margin-top:2%">
                            <option value="0" disabled selected>เลือกอาจารย์</option>
                            <?php
                            $max = 0;
                            while ($result = mysqli_fetch_array($group, MYSQLI_ASSOC)) {  ?>

                                    <option value="<?php echo $result['Teacher_Id'] ?>">
                                        <h5 style="margin-top:5%;color:black" align='center'><?php echo $result['Teacher_Name']?> </h5>
                                    </option>
